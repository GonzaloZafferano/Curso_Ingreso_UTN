...
function mostrar() {
  ...
  var acumuladorPreciosTotalesSinDto;
  var importeTotalAPagarConDto;
  var acumuladorArena;
  var acumuladorCal;
  var acumuladorCemento;
  var masCantidadDeBolsasTipo;
  var masCantidadDeBolsasUnidades;
  var banderaMasCaro;
  var masCaroPrecio;
  var masCaroTipo;
  ...
  acumuladorPreciosTotalesSinDto = 0;
  acumuladorArena = 0;
  acumuladorCal = 0;
  acumuladorCemento = 0;

  do {
    tipoProducto = prompt("Por favor, ingrese el tipo de producto: 'arena', 'cal', 'cemento'");
    tipoProducto = tipoProducto.toLowerCase();

    while (tipoProducto != "arena" && tipoProducto != "cal" && tipoProducto != "cemento") {
      tipoProducto = prompt("Dato invalido. Por favor, ingrese el tipo de producto: 'arena', 'cal', 'cemento'");
      tipoProducto = tipoProducto.toLowerCase();
    }

    precioPorBolsas = prompt("Por favor, ingrese el precio por bolsa.");
    precioPorBolsas = parseInt(precioPorBolsas);

    while (precioPorBolsas < 1 || isNaN(precioPorBolsas)) {
      precioPorBolsas = prompt("Dato invalido. Por favor, ingrese el precio por bolsa.");
      precioPorBolsas = parseInt(precioPorBolsas);
    }

    cantidadBolsas = prompt("Por favor, ingrese la cantidad de bolsas.");
    cantidadBolsas = parseInt(cantidadBolsas);

    while (cantidadBolsas < 1 || isNaN(cantidadBolsas)) {
      cantidadBolsas = prompt("Dato invalido. Por favor, ingrese la cantidad de bolsas.");
      cantidadBolsas = parseInt(cantidadBolsas);
    }
    precioTotalPorCompra = cantidadBolsas * precioPorBolsas;
    acumuladorPreciosTotalesSinDto += precioTotalPorCompra;
    ...
    if (precioPorBolsas > masCaroPrecio || banderaMasCaro == 0) {
      banderaMasCaro = 1;
      masCaroPrecio = precioPorBolsas;
      masCaroTipo = tipoProducto;
    }

    acumuladorDeBolsas += cantidadBolsas;
  ...
  mensajeA = "A - El total a pagar sin descuento es de " + acumuladorPreciosTotalesSinDto + "\n";

  if (descuento != 0) {
    importeTotalAPagarConDto = acumuladorPreciosTotalesSinDto - acumuladorPreciosTotalesSinDto * descuento / 100;
    mensajeB = "B - El total a pagar con descuento es de $" + importeTotalAPagarConDto + "\n";
  }
  else {
    mensajeB = "B - No hubo descuento. La cantidad de bolsas fue de " + acumuladorDeBolsas + "\n";
  }

  if (acumuladorArena > acumuladorCal && acumuladorArena > acumuladorCemento) {
    masCantidadDeBolsasTipo = "Arena";
    masCantidadDeBolsasUnidades = acumuladorArena;

  }
  else {
    if (acumuladorCal > acumuladorCemento) {
      masCantidadDeBolsasTipo = "Cal";
      masCantidadDeBolsasUnidades = acumuladorCal;

    }
    else {
      masCantidadDeBolsasTipo = "Cemento";
      masCantidadDeBolsasUnidades = acumuladorCemento;

    }
  }

  mensajeC = "C - El tipo con mas unidades es " + masCantidadDeBolsasTipo + " con " + masCantidadDeBolsasUnidades + " unidades \n";
  mensajeD = "D - El tipo mas caro es " + masCaroTipo + " y su precio es de $" + masCaroPrecio;
